...
def validate(s):
    ...
    validate a password
    """
    spec_str, pass_str = [a.strip() for a in s.split(':')]
    occ_str, letter = [a.strip() for a in spec_str.split(' ')]
    min_occ, max_occ = [int(a.strip()) for a in occ_str.split('-')]
    if pass_str.count(letter) >= min_occ and pass_str.count(letter) <= max_occ:
        return True
...
def validate_2(s):
    ...
    """
    spec_str, pass_str = [a.strip() for a in s.split(':')]
    pos_str, letter = [a.strip() for a in spec_str.split(' ')]
    pos_1, pos_2 = [int(a.strip()) for a in pos_str.split('-')]
    pos_1 -= 1
    pos_2 -= 1
    if (pass_str[pos_1] == letter or pass_str[pos_2] == letter) and pass_str[pos_1] != pass_str[pos_2]:
        return True
    return False
