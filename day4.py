...
hcl_pattern = re.compile(r'^#[0-9,a-f]{6}$')

required_record_fields = {"byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"}
valid_ecl_values = {"amb", "blu", "brn", "gry", "grn", "hzl", "oth"}

def parse_record(record):
    """
    return a dict of a parsed record
    """
    record_dict = {}
    ...
    for field in record.strip().split(' '):
        k, v = field.split(':')
        record_dict[k] = v
    record_dict['isvalid'] = is_valid_record(record_dict)
    return record_dict
...
def is_valid_ecl(ecl):
    return ecl in valid_ecl_values
...
def is_valid_record(record):
    """
    return a boolean for whether a record is valid
    """
    if not required_record_fields.issubset(set(record.keys())):
        return False
    if not is_valid_byr(record['byr']):
        return False
    if not is_valid_iyr(record['iyr']):
        return False
    if not is_valid_eyr(record['eyr']):
        return False
    if not is_valid_hgt(record['hgt']):
        return False
    if not is_valid_hcl(record['hcl']):
        return False
    if not is_valid_ecl(record['ecl']):
        return False
    if not is_valid_pid(record['pid']):
        return False
    return True

data = data.split('\n\n')
data = [parse_record(record.strip()) for record in data]
valid_records = 0
